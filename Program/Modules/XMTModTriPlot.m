function varargout = XMTModTriPlot(varargin)
% XMTFunctionsInfos call the GUI VER_XMTModTriPlot_XXX
...
if TheCodeVersion >= 8040
    disp(['Compatible version: VER_XMTModTriPlot_804']);
    disp(' ')
    eval(['VER_XMTModTriPlot_804(',char(Instruction),')']);
else
    disp(['Compatible version: VER_XMTModTriPlot_750']);
    disp(' ')
    eval(['VER_XMTModTriPlot_750(',char(Instruction),')']);
end
