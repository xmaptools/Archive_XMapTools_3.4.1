function varargout = XMTModBinary(varargin)
% XMTFunctionsInfos call the GUI VER_XMTModBinary_XXX
...
if TheCodeVersion >= 8040
    disp(['Compatible version: VER_XMTModBinary_804']);
    disp(' ')
    eval(['VER_XMTModBinary_804(',char(Instruction),');']);
else
    disp(['Compatible version: VER_XMTModBinary_750']);
    disp(' ')
    eval(['VER_XMTModBinary_750(',char(Instruction),');']);
end
