function varargout = XMTModIDC(varargin)
...
if TheCodeVersion >= 8040
    disp(['Compatible version: VER_XMTModIDC_804']);
    disp(' ')
    eval(['[a,b] = VER_XMTModIDC_804(',char(Instruction),');']);
else
    disp(['Compatible version: VER_XMTModIDC_750']);
    disp(' ')
    eval(['[a,b] = VER_XMTModIDC_750(',char(Instruction),');']);
end
