function [Si,Ti,Al,V,Cr,Fe3,Fe2,Mn,Mg,Ca,H2,SumTetra,SumOcta,SumDod,SumCharges_vect,FeO,Fe2O3,Total] = StructFctGarnetH2O_OctOpti(Data,handles)
% -
% XMapTools External Function: Structural formula of Garnet (including Fe3+ estimate)
%
%   [outputs] = function_Name(Data,handles);
%
%   1>Garnet_H2O>Grt_Meth1-OctOpti>StructFctGarnetH2O_OctOpti>Si Ti Al V Cr Fe3 Fe2 Mn Mg 
...
%
%   *** Method 1 ***    
%   OctOpti (Fe3+ is approximated on the basis of Sum(OctahedralSite) = 2; 
...
for i=1:NbSamples
    ...
    if sum(Data(i,:)) > 0.1  % Is garnet
        ...
        if method==1;
            FeOt=Data(i,6);
            Fe3_c=2-sum(Normalisation_Cbasis([2 3 4 5]));
            Fe2O3_c=Fe3_c/CationicFactor/2*M_Fe2O3;
        ...
        if method==1 | method==2; %Fe2 in Tetra
        ...
        elseif method==3; %Fe2 in oct
            SumTetra_Cbasis=sum(Normalisation_Cbasis([1 11]));
            SumOcta_Cbasis=sum(Normalisation_Cbasis([2 3 4 5 6 7]));
            SumDod_Cbasis=sum(Normalisation_Cbasis([8 9 10]));
        end
