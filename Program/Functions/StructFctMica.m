function [Si4,Al_T2,Si_T2,V_M1,Fe_M1,Mg_M1,Al_M2M3,Mg_M2M3,Fe_M2M3,XMg,XFe,K_A,V_A,Na_A,Sum_A,XBt,XCel,XMs,XPrl,XPg,Xsum] = StructFctPhengite(Data,handles)
% -
% XMapTools External Function: Structural formula of mica 
%
...
for i=1:length(Data(:,1)) % one by one
    ...
    if OnCal
        ...
        if Si_T2(i)-1-XPrl(i) > 0
            XCel(i) = Si_T2(i)-1-XPrl(i);
        end
        if XBt(i)+XPg(i)+XPrl(i)+XCel(i) < 1
            XMs(i) = 1 - (XBt(i)+XPg(i)+XPrl(i)+XCel(i));
        end
...
%         Ncel = FeMg_M2M3;
%          
%         XBt(i) = NBt/(NBt+Npar+Nmu+Npyr+Ncel);
%         XPg(i) = Npar/(NBt+Npar+Nmu+Npyr+Ncel);
%         XMs(i) = Nmu/(NBt+Npar+Nmu+Npyr+Ncel);
%         XPrl(i) = Npyr/(NBt+Npar+Nmu+Npyr+Ncel);
%         XCel(i) = Ncel/(NBt+Npar+Nmu+Npyr+Ncel);
%         
...
%         XMus(i) = (Al_M2M3(i) - (2*XPrl(i) + XCel(i)))/2; 
        ...
        Xsum(i) = XBt(i) + XPg(i)+XCel(i)+XPrl(i)+XMs(i); 
