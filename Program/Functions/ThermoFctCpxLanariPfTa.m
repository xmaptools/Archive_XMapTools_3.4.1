function [P,Tf] = ThermoFctCpxLanariPfTa(Data,handles)
% -
...
%
%
% [Values] = ThermoFctCpxLanariPTb(Data);
%
...
%
% Setup : 
% 1>Clinopyroxene>Cpx P-T (Cpx-Gar-Phg)>ThermoFctCpxLanariPfTa> 
...
lesDef = {'6.4','2.35','1.23','3.01','0.78','1.72'};
...
lesNames = {'Na-Cpx','interp limits','Temperature Values (°C)'};
lesDef = {'0.1 0.3 0.4','0 0.001 0.6','640 680 700'};
Values = inputdlg(lesNames,'Input',1,lesDef);

X = str2num(char(strread(Values{1},'%s')));
Y = str2num(char(strread(Values{3},'%s')));
lesLim = str2num(char(strread(Values{2},'%s')));
%leFactor = str2num(char(strread(Values{4},'%s')));

Xi = lesLim(1):lesLim(2):lesLim(3);
...
for i=1:length(Data(:,1)) % one by one
    ...
    if OnCal
        ...
        % T value....
        [laVal,laOu] = min(abs([Xi-Na_cpx]));
        T = Yi(laOu); %+randn(1)*leFactor;
